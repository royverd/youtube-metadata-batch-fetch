...
**Every irreplaceable file has read-only copies.** Whenever `metadata.json`, `progress.json` or the screening markdown changes, a copy named like `metadata.20260915-183000.json` goes to `data/backups/` and is made read-only. Identical content isn't copied twice, and the program never alters or deletes a copy. Settings → Backups shows the counts and has a Clear old copies button, which keeps the newest of each file; the sidebar shows a notice once any file has more copies than the threshold you set (15 by default). Copies of the screening markdown made during a terminal session are taken on the app's 5-second poll, and on startup for anything changed while the app was closed.
...
| `data/backups/` | the app | Read-only copies of `metadata.json`, `progress.json` and the screening markdown, one per change, kept until cleared from Settings |
